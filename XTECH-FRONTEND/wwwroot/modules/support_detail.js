var userTicket = {
    ...
    initSignalR: function () {
        ...
        this.connection = new signalR.HubConnectionBuilder()
            .withUrl(window.userTicketEndpoints.hubUrl)
            .withAutomaticReconnect()
            .build();

        this.connection.on("ReceiveMessage", function (m) {
            var msg = userTicket.normalizeMessage(m);
            if (!msg) return;
        ...
        // nếu BE/API có bắn event attachments riêng
        this.connection.on("ReceiveAttachments", function (p) {
            if (!p) return;
            var messageId = p.messageId || p.MessageId;
            var files = p.attachFiles || p.AttachFiles || [];
            if (!messageId || !files.length) return;

            userTicket.updateMessageAttachments(messageId, files);
        });

        this.connection.start()
            .then(() => this.connection.invoke("JoinTicket", userTicket.ticketId))
            .then(() => $('#rtStatus').text('Realtime: Online'))
            .catch(err => {
                console.error(err);
                $('#rtStatus').text('Realtime: Offline');
            });

        this.connection.onreconnecting(() => $('#rtStatus').text('Realtime: Reconnecting...'));
        this.connection.onreconnected(() => $('#rtStatus').text('Realtime: Online'));
        this.connection.onclose(() => $('#rtStatus').text('Realtime: Offline'));
    },
    ...
    sendReply: function () {
        var content = ($('#txtReply').val() || '').trim(); // HTML từ replyEditor.beforeSend()
        ...
        if (files && files.length) {
            for (var i = 0; i < files.length; i++) {
                fd.append('attachFiles', files[i]); // key match controller
            }
        }
    ...
    updateMessageAttachments: function (messageId, files) {
        ...
        var attHtml = this.renderAttachments(files);
        $msg.append(attHtml);
    },
